  ...
  if (isset($_POST['register'])) {
    ...
    if (($user['email'] === $email)) {
     ...
     $query = "INSERT INTO user (user_id, first_name, last_name, email, password, user_type, activation_code, email_status, created_at, updated_at)
      ...
      // print_r(  $last_id );
      $query1  = "INSERT INTO profile (profile_id, description, psw_hint, created_at, updated_at, user_id) 
                                      VALUES (NULL, NULL, NULL,  current_timestamp(),  current_timestamp(), '$last_id')";
...
//password reset function
    if(isset($_POST['forgot'])){
      ...
      //  get user profile
       ...
       $query_get_profile = "SELECT * FROM profile WHERE user_id = '$user_profile_id' LIMIT 1";
       $result = mysqli_query($conn, $query_get_profile);
        ...
        $result = mysqli_query($conn, $query);
          if(isset($result))	{
            ...
            $base_url="http://localhost/ticket/account/reset.php?key=".$email."&reset=".$password."'</a>";
              ...
              $mail_body .= '<p  style="color:black;"> Find your password hint below. </p><br>';
              $mail_body .= "<span>".$profile['psw_hint'].",  </span>";
